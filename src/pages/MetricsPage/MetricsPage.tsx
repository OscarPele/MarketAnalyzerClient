...
export default function MetricsPage() {
  ...
  async function load(s: string) {
    ...
    setError(null);
    try {
      const [r200, r50, r21, rSlope, rStruct] = await Promise.all([
        fetchEma200(s),
        fetchEma50(s),
        fetchEma21(s),
        fetchEma200Slope(s),
  ...
  const arrow = (s: Slope | null) => {
    if (!s) return "—";
    return s.sign === "up" ? "↑" : s.sign === "down" ? "↓" : "→";
  };
  ...
  return (
    <main className="metrics-root">
      ...
      <section className="metrics-grid">
        <div className="card">
          <h2>EMA 200 · 1h</h2>
          <div className="value">{fmt(ema1h)}</div>
        </div>
        <div className="card">
          <h2>EMA 200 · 4h</h2>
          <div className="value">{fmt(ema4h)}</div>
        </div>
        <div className="card">
          <h2>EMA 50 · 1h</h2>
          <div className="value">{fmt(ema50_1h)}</div>
        </div>
        <div className="card">
          <h2>EMA 21 · 1h</h2>
          <div className="value">{fmt(ema21_1h)}</div>
        </div>
        <div className="card">
        ...
        <div className="card">
          <h2>Estructura 1h</h2>
          <div className="value">{biasText}</div>
